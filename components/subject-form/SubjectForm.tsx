...
interface SubjectFormProps {
  onAddSubject: (formData: SubjectFormData) => void;
  title: string;
}
...
export default function SubjectForm({ onAddSubject, title }: SubjectFormProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const transformDays = (days: string[]): string[] => {
      const dayMap = new Map(
        DAYS_OF_WEEK.map((day) => [day.id.toLowerCase(), day.label])
      );
      return days.map((day) => dayMap.get(day.toLowerCase()) || day);
    };

    // Add submission logic here
    onAddSubject({
      name: subjectTitle,
      lesson_count: numberOfLessons,
      teaching_days: [...transformDays(selectedDays)],
    });
